...
import { FlexibleScorecard } from "../game/FlexibleScorecard";
import { LogOut, Plus, Spade } from "lucide-react";
import { Game, GameSetup } from "@/types/game";
import { useFlexibleGames } from "@/hooks/useFlexibleGames";
...
export const FlexibleDashboard = ({ user, onLogout }: FlexibleDashboardProps) => {
  ...
  const [sort, setSort] = useState({ key: "createdAt", order: "desc" as "desc" | "asc" });
  ...
  const filteredAndSortedGames = games
    ...
    .filter((game) => {
      ...
      return game.teams.some(team => 
        ...
        team.players.some(p => p.toLowerCase().includes(lowercasedTerm))
      );
    })
    .sort((a, b) => {
      ...
      if (sort.key === "teamName") {
        const nameA = a.teams.map(t => t.name).join(" vs ");
        const nameB = b.teams.map(t => t.name).join(" vs ");
        return sort.order === "asc"
      ...
      if (sort.key === "score") {
        const scoreA = a.finalScores
          ? Object.values(a.finalScores).reduce((sum, score) => sum + score, 0)
          : 0;
        const scoreB = b.finalScores
          ? Object.values(b.finalScores).reduce((sum, score) => sum + score, 0)
          : 0;
  ...
  const handleNewGame = async (setup: GameSetup) => {
    try {
      const newGame = await createGame(setup);
  ...
  const handleUpdateRound = async (
    ...
    await updateRound(gameId, roundNumber, teamId, bid, won, bags, score);
  };
